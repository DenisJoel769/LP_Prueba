<template>
  <v-app id="app">
    <v-app-bar :height="`${appBarHeight}`" app class="navbar" dark :clipped-right="true">
      <div
        class="ml-md-5 ml-lg-5 mr-sm-10 mr-lg-0 mr-md-0"
        style="width: 15vh; cursor: pointer"
      >
        <v-img src="@/assets/logo-laplata-min.png"></v-img>
      </div>
      <div class="vertical-line"></div>
      <div class="ml-md-5 mb-md-n3">
        <v-menu offset-y :close-on-content-click="false" disabled>
      ...
      <div class="vertical-line"></div>
      <div class="ml-md-n10">
        <buscador-menu :disabled="true"></buscador-menu>
      </div>
      <div class="vertical-line"></div>
      ...
      <div class="ml-2">
        <v-btn disabled icon small color="#ffffff"><v-icon>mdi-settings</v-icon></v-btn>
      </div>
      <v-spacer></v-spacer>
      <div class="vertical-line"></div>
      <div class="text-body-2 d-inline ml-md-11" style="color: white">Cerrar Sesión</div>
      <div class="d-inline">
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn disabled icon v-on="on" @click="AUTH_LOGOUT({})">
    ...
    <v-container>
      <div class="d-flex justify-end mb-n4">
        ...
        <div class="d-inline mt-n1 ml-1">
          <v-icon color="#f9c701">mdi-star</v-icon>
        </div>
        <div class="d-inline ml-1" style="color: #8a8a8a">/</div>
        <div class="d-inline mt-n1 ml-1">
          <v-icon color="#8a8a8a">mdi-help</v-icon>
        </div>
      </div>
      <router-view />
    </v-container>
...
import DrawerContact from "@/components/ContactCenter/DrawerContact.vue";
...
export default {
  ...
  methods: {
    ...
    selectItem(e) {
      if (e.itemData.path != "" || e.itemData.path != null) {
